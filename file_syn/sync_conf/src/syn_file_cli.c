...
static char *syn_sock_conf_path = "/storage/hawk/ha/sync_config/sync_sock.conf";

/* the client label name in Net config file */
static char *syn_sock_conf_label = "syncli";

/* the server ip to be connected in Net config file */
static char *syn_sock_conf_ip = "sync_sock_cli_ip";

/* the server port to be connected in Net config file */
static char *syn_sock_conf_port = "sync_sock_cli_port";
...
int sync_cli_send_file_data(char *send_buf, int send_len);
...
sync_conf_labels_t *sync_cli_load_conf_label(sync_conf_labels_t *p_sync_conf_labels_t, char *label_name)
{
...
static sync_conf_labels_t *sync_cli_deal_label_line(char *read_line, sync_conf_labels_t **p_conf_head, sync_conf_labels_t **p_conf_tail)
{
    char name[64] = {0};
...
static sync_conf_items_t *sync_cli_deal_item_line(char *read_line, sync_conf_labels_t **p_conf_que, sync_conf_items_t **p_item_tail)
{
    char name[64] = {0};
    char value[512] = {0};
...
void dis_args(char **args)
{
	...
	while (args[i] != NULL)
	{
		printf("ÎÄ¼þÍ¬²½Êä³ö²ÎÊýÁÐ±í·Ö±ðÎª£º[%d]: %s\n", i, args[i]);
		++i;
	}
}
#endif
...
int sync_cli_pkg_single_file(char* file_name)
{
	...
	data_md5 = (unsigned char*)(send_buf + proto_len);
	sync_cli_md5_update (&mdContext, data_md5, fdata_len);
	...
	if (sync_cli_send_file_data(send_buf, send_len) == false)
	{
		printf("ÎÄ¼þÍ¬²½·¢ËÍÊý¾ÝÊ§°Ü£¡\n");
		return false;
	}
...
int sync_cli_read_sock_conf()
{
	...
	if ((conf = sync_cli_load_conf_file(syn_sock_conf_path)) == NULL)
	{
		printf("¼ÓÔØÅäÖÃÎÄ¼þÍ¬²½ÍøÂçÅäÖÃÎÄ¼þÊ§°Ü!\n");
        return false;
	}

	if ((general = sync_cli_load_conf_label(conf, syn_sock_conf_label)) == NULL)
	{
		printf("¼ÓÔØÅäÖÃÎÄ¼þÍ¬²½ÍøÂçÅäÖÃÎÄ¼þ±êÇ©ÏîÊ§°Ü!\n");
		sync_cli_free_conf_file(&conf);
		return false;
	}
	
	if ((val = sync_cli_get_label_value(general, syn_sock_conf_ip)) != NULL)
	{
        sync_sock_addr.s_ip = sync_cli_trans_inet_atoul(val);
	}

	if ((val = sync_cli_get_label_value(general, syn_sock_conf_port)) != NULL)
	{
...
int main(int argc, char **argv)
{
	...
	{
		printf("%s\n", "ÎÄ¼þÍ¬²½¼ÓÔØÅäÖÃÎÄ¼þ´íÎó!");
        return false;
	}

	if ((general = sync_cli_load_conf_label(mod_option_conf, sync_mod_conf_label)) == NULL)
	{
		printf("%s\n","ÎÄ¼þÍ¬²½¶ÁÈ¡ÅäÖÃÎÄ¼þ´íÎó");
