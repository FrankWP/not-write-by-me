 ...
 * Current Version: 1.0
 * Author£º wangyonga
 * Creation Date£º2015.1.29
 * Modify History£ºNone
 *
 *
 * *******************************************************/
...
#define SYNC_MOD_NAME	"ÎÄ¼şÍ¬²½"

#define SYNC_LOGNT_START_SER	"ÎÄ¼şÍ¬²½·şÎñ¶Ë·şÎñ¿ªÆô"
#define SYNC_LOGNT_FINISH_RECV	"ÎÄ¼şÍ¬²½·şÎñ¶Ë½ÓÊÕÅäÖÃÎÄ¼şÍê±Ï"

#define SYNC_LOGERR_READ_LABEL	"ÎÄ¼şÍ¬²½¶ÁÈ¡ÅäÖÃÎÄ¼ş±êÇ©´íÎó"
#define SYNC_LOGERR_LOAD_LABEL	"ÎÄ¼şÍ¬²½¼ÓÔØÅäÖÃÎÄ¼ş´íÎó!"
#define SYNC_LOGERR_LOAD_ITEM	"ÎÄ¼şÍ¬²½¼ÓÔØÅäÖÃÁĞ±íÎÄ¼şÊ§°Ü»òÎÄ¼şÎª¿Õ!"

#define SYNC_LOGERR_FILE_DEAL		"ÎÄ¼şÍ¬²½´¦ÀíÍ¬²½ÎÄ¼şÊ§°Ü!"
#define SYNC_LOGERR_FILE_CANT_READ	"ÎÄ¼şÍ¬²½ÎÄ¼şÎŞ·¨´ò¿ª"
#define SYNC_LOGERR_FILE_BUF_MALLOC	"ÎÄ¼şÉêÇëÄÚ´æÊ§°Ü!"
#define SYNC_LOGERR_FILE_MD5_CHANGE	"ÎÄ¼şÍ¬²½MD5Öµ·¢ÉúÁË±ä»¯£¡"
#define SYNC_LOGERR_FILE_FINISH_RECV "ÎÄ¼şÍ¬²½ÎÄ¼şÍ¬²½Íê±Ï£¡"
#define SYNC_LOGERR_FILE_REP_FAIL   "ÎÄ¼şÌæ»»´íÎó£¡"
#define SYNC_LOGNT_FILE_REP_SUCC   "ÎÄ¼şÌæ»»³É¹¦£¡"

#define SYNC_LOGERR_SOCK_SEND	"ÎÄ¼şÍ¬²½ÍøÂç·¢ËÍÊı¾İÊ§°Ü!"
#define SYNC_LOGERR_SOCK_INIT	"ÎÄ¼şÍ¬²½³õÊ¼»¯Ì×½Ó×ÖÊ§°Ü!"
#define SYNC_LOGERR_SOCK_SETSOCKOPT	"ÎÄ¼şÍ¬²½³õÊ¼»¯ÉèÖÃÌ×½Ó×Ö²ÎÊıÊ§°Ü!"
#define SYNC_LOGERR_SOCK_BIND	"ÎÄ¼şÍ¬²½³õÊ¼»¯ÉèÖÃ°ó¶¨ÍøÂç¶Ë¿ÚÊ§°Ü!"
#define SYNC_LOGERR_SOCK_LISTEN	"ÎÄ¼şÍ¬²½³õÊ¼»¯ÉèÖÃ¼àÌıÍøÂç¶Ë¿ÚÊ§°Ü!"
#define SYNC_LOGERR_SOCK_ACCEPT "ÎÄ¼şÍ¬²½³õÊ¼»¯½ÓÊÕÍøÂçÁ¬½ÓÊ§°ÜÜ!"
#define SYNC_LOGERR_SOCK_RECV	"ÎÄ¼şÍ¬²½³õÊ¼»¯½ÓÊÕÍøÂçÊı¾İÊ§°Ü!"
#define SYNC_LOGERR_SOCK_CONN	"ÎÄ¼şÍ¬²½ÎŞ·¨½øĞĞÍøÂçÁ¬½Ó!"
#define SYNC_LOGERR_SOCK_LOAD_LABEL	"ÎÄ¼şÍ¬²½¼ÓÔØÍøÂçÅäÖÃÎÄ¼ş´íÎó!"
#define SYNC_LOGERR_SOCK_READ_LABEL	"ÎÄ¼şÍ¬²½¼ÓÔØÍøÂçÅäÖÃÁĞ±íÎÄ¼şÊ§°Ü»òÎÄ¼şÎª¿Õ!"
...
static char sync_sock_conf_label[] = "synser";

/* the server ip value in Net config file */
static char sync_sock_conf_ip[] = "sync_sock_ser_ip";

/* the server port value in Net config file */
static char sync_sock_conf_port[] = "sync_sock_ser_port";
...
int sync_ser_write_data_tofile(char *buf_head, char *buf_data, int file_len);
...
int sync_ser_replace_conf_file(char *fsrc, char *fdst)
{
	...
	if (ret == 0) 
	{
		...
		dmanage_log_notice(PRE_HAWK_LOG "act=\"%s\" msg=\"%s\" filename=\"%s\" " , 
			SYNC_MOD_NAME, SYNC_LOGNT_FILE_REP_SUCC, fdst);
	}
	else
	{
		ret = false;
...
int sync_ser_init_sock_conf(unsigned int s_ip, unsigned short s_port)
{
	...
	{
		//printf("ÎÄ¼şÍ¬²½ÉèÖÃÍøÂç²ÎÊı´íÎó£º%s", strerror(errno));
		dmanage_log_err(PRE_HAWK_LOG "act=\"%s\" msg=\"%s\" " , 
			SYNC_MOD_NAME, SYNC_LOGERR_SOCK_SETSOCKOPT);
    ...
    {  
        //printf("ÎÄ¼şÍ¬²½·şÎñÆ÷°ó¶¨¶Ë¿Ú: %d Ê§°Ü!\n", server_addr.sin_port);  
		dmanage_log_err(PRE_HAWK_LOG "act=\"%s\" msg=\"%s\" " , 
			SYNC_MOD_NAME, SYNC_LOGERR_SOCK_BIND);
		return false;
    }  
    ...
    {  
        //printf("ÎÄ¼şÍ¬²½·şÎñÆ÷¼àÌıÊ§°Ü!\n");  
		dmanage_log_err(PRE_HAWK_LOG "act=\"%s\" msg=\"%s\" " , 
			SYNC_MOD_NAME, SYNC_LOGERR_SOCK_LISTEN);
...
int sync_ser_read_sock_conf()
{
	...
	if ((conf = sync_ser_load_conf_file(sync_sock_conf_path)) == NULL)
	{
		//printf("¶ÁÈ¡ÎÄ¼şÍ¬²½ÍøÂçÅäÖÃÊ§°Ü£¡\n");
	...
	if ((general = sync_ser_load_conf_label(conf, sync_sock_conf_label)) == NULL)
	{
	...
	if ((val = sync_ser_get_label_value(general, sync_sock_conf_ip)) != NULL)
	{
        sync_sock_addr.s_ip = sync_ser_trans_inet_atoul(val);
	}

	if ((val = sync_ser_get_label_value(general, sync_sock_conf_port)) != NULL)
	{
...
int sync_ser_recv_fdata()
{
	...
	{
		//printf("ÎÄ¼şÍ¬²½³õÊ¼»¯ÍøÂçÅäÖÃÊ§°Ü\n");
		return false;
	}

	dmanage_log_notice(PRE_HAWK_LOG "act=\"%s\" msg=\"%s\"", 
			SYNC_MOD_NAME, SYNC_LOGNT_START_SER);

	while (1)
	{
		puts("wait------------------");
		int recv_len = 0;
		char buf_head[BUFFER_SIZE] = {0};  
		char *buf_data = NULL;  
		char *file_buf = NULL;
        ...
        {  
            //printf("ÎÄ¼şÍ¬²½½ÓÊÕÍøÂçÁ¬½ÓÊ§°Ü!\n");  

			dmanage_log_err(PRE_HAWK_LOG "act=\"%s\" msg=\"%s\" " , 
				SYNC_MOD_NAME, SYNC_LOGERR_SOCK_ACCEPT);
            continue;  
        }  
		puts("after accept");

        length = recv(new_server_socket, buf_head, sizeof(sync_pkg_head_t), 0);  
        if (length < 0)  
        {  
			close(new_server_socket);
            //printf("ÎÄ¼şÍ¬²½·şÎñÆ÷½ÓÊÜÎÄ¼şÊı¾İÊ§°Ü!\n");  

			dmanage_log_err(PRE_HAWK_LOG "act=\"%s\" msg=\"%s\" " , 
				SYNC_MOD_NAME, SYNC_LOGERR_SOCK_RECV);

            continue;  
        }  
		if (!strcmp(buf_head, SYNC_PKG_HEAD_FLG))
		{
			close(new_server_socket);
			continue;
		}
		t_disbuf(buf_head, length);

		sync_pkg_head_t *pkg_head = (sync_pkg_head_t *)buf_head;
		if (pkg_head -> sync_pkg_file_cont_len > 0)
		{
			buf_data = (char*)malloc( pkg_head -> sync_pkg_file_cont_len );
			if (buf_data == NULL) 
			{
				close(new_server_socket);
				//printf("½ÓÊÜÎÄ¼şÉêÇëÄÚ´æÊ§°Ü£¡:%d\n", total_len);  

				dmanage_log_err(PRE_HAWK_LOG "act=\"%s\" msg=\"%s\" " , 
						SYNC_MOD_NAME, SYNC_LOGERR_FILE_BUF_MALLOC);
				continue;  
			}

			do 
			{
				length = recv(new_server_socket, buf_data, pkg_head -> sync_pkg_file_cont_len, 0);
				if (length < 0)
				{
					break;
				}
				recv_len = recv_len + length;
			} while (recv_len != pkg_head->sync_pkg_file_cont_len);
			t_disbuf(buf_data, recv_len);
			sync_ser_write_data_tofile(buf_head, buf_data, recv_len);
		}	
...
int sync_ser_write_data_tofile(char *buf_head, char *buf_data, int file_len)
{
	...
	if ((buf_head == NULL)||(buf_data == 0))
	{
		//printf("ÎÄ¼şÍ¬²½ÅäÖÃÎÄ¼ş²»´æÔÚ!\n");
		return false;
	}

	t_disbuf(buf_head, sizeof(sync_pkg_head_t));
	sync_pkg_head_t *pkg_head = (sync_pkg_head_t *)buf_head;	
	fname_len = pkg_head->sync_pkg_file_name_len;
	fdata_len = pkg_head->sync_pkg_file_cont_len;
	memcpy(fname, pkg_head->sync_pkg_file_name, fname_len);
	memcpy(md5_src, pkg_head->sync_pkg_file_md5, 32);
    ...
    if (fp == NULL)  
    {  
        //printf("ÎÄ¼şÍ¬²½ÎÄ¼ş:\t%s ÎŞ·¨´ò¿ª¶ÁĞ´!\n", fname);  

		dmanage_log_err(PRE_HAWK_LOG "act=\"%s\" msg=\"%s\" filename=\"%s\" " , 
			SYNC_MOD_NAME, SYNC_LOGERR_FILE_CANT_READ, fname);
	...
	{
		write_len = fwrite(buf_data, sizeof(char), fdata_len, fp);  
	...
	{
		printf("ftmp:%s fname:%s \n", ftmp, fname);
		if (sync_ser_replace_conf_file(ftmp, fname) < 0)
		{
			dmanage_log_err(PRE_HAWK_LOG "act=\"%s\" msg=\"%s\" filename=\"%s\" " , 
			SYNC_MOD_NAME, SYNC_LOGERR_FILE_REP_FAIL, fname);
		...
		{
			dmanage_log_err(PRE_HAWK_LOG "act=\"%s\" msg=\"%s\" filename=\"%s\" " , 
			SYNC_MOD_NAME, SYNC_LOGERR_FILE_FINISH_RECV, fname);
		}
	}
	else
	{
		//printf("ÎÄ¼şÍ¬²½MD5Öµ²»Í¬\n");
		dmanage_log_err(PRE_HAWK_LOG "act=\"%s\" msg=\"%s\" filename=\"%s\" " , 
			SYNC_MOD_NAME, SYNC_LOGERR_FILE_MD5_CHANGE, fname);
