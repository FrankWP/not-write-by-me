/********************************************************
 *  create:     2012/05/23
 *  match file: vp_h3c_harbin.c of CLiaoNing
 *******************************************************/
...
{ 
  ...
  if(IS_TMS)
    inet_ultoa(__gg.inner_addr,rip);
  else
  ...
  /*Replace server ip*/
  if(IS_TMS)
    inet_ultoa(__gg.outer_addr,saddr);
  else
 ...
 *Do reply
 *********************************************************/
int h3c_harbin_reply(pvp_uthttp put, char **ppbuf, u32 *pack_len)
{
  int rtn = 1;
 ...
 * Do request
 *************************************************************/
int h3c_harbin_request(pvp_uthttp put, char **ppbuf, u32 *pack_len)
{ 
  int rtn = 1;
 ...
 *Do scoket
 *********************************************************/
 int h3c_harbin_socket(pvp_uthttp put, const int sockfd)
 {
  u16 xport;
  SAI xaddr;
  int i;
